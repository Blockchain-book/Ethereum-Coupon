...
//        resultIntent.setClass(this, TestActivity.class);
...
//        CaptureActivity.this.finish();
        ...
        switch (type){
            case "present":
                resultIntent.setClass(this, PresentActivity.class);
                bundle.putString("result",rawResult.getText());
                resultIntent.putExtras(bundle);
                startActivity(resultIntent);
                break;
            case "pay":
                final String merchantId = rawResult.getText();
                final String requestString = "merchantId="+merchantId;
                String url = UrlManager.createUrlString("/merchant/queryMerchantNameById.action");
                HttpTaskTool httpTaskTool = new HttpTaskTool(requestString,url);
                    ...
                    public void onDataReceivedSuccess(String data) {
                        if (data!=null && !"".equals(data)){
                            try {
                                ...
                                String result = jsonObject.getString("merchantName");
                                NearbyMerchantItem nearbyMerchantItem = new NearbyMerchantItem(merchantId,result);
                                if (result!=null && !"".equals(result)){
                                    resultIntent.setClass(CaptureActivity.this, ConfirmPayActivity.class);
                                    bundle.putSerializable("merchantInfo",nearbyMerchantItem);
                                    resultIntent.putExtras(bundle);
                                    startActivity(resultIntent);
                                }
                ...
                break;
            case "getCoupon":
                String merchantID = rawResult.getText();
                SharedPreferences sharedPreferences = getSharedPreferences("data",MODE_PRIVATE);
                String consumerId = sharedPreferences.getString("consumerId","");
                String getCouponUrl = UrlManager.createUrlString("/consumer/applyCoupon.action");
                JSONObject jsonObject = new JSONObject();
                try {
                    jsonObject.put("merchantId",merchantID);
                    jsonObject.put("consumerId",consumerId);
                    final String request = jsonObject.toString();
                    HttpTaskForJsonTool httpTaskForJsonTool = new HttpTaskForJsonTool(request,getCouponUrl);
                    httpTaskForJsonTool.execute();
                    httpTaskForJsonTool.setOnAsyncResponse(new AsyncResponse() {
                        @Override
                        public void onDataReceivedSuccess(String data) {
                            if(data!=null && !"".equals(data)){
                                try {
                                    JSONObject jsonObject1 = new JSONObject(data);
                                    String res = jsonObject1.getString("resultCode");
                                    if ("1".equals(res)){
                                        res = "申请优惠券信息已发送给商家";
                                    }else {
                                        res = "申请优惠券信息发送失败";
